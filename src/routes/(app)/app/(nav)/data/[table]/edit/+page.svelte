<script lang="ts">
  ...
  export let data
  let { table, columns, user_facing_name, table_description } = data

  // Create a store for the column list.
  let columnList = writable(
    columns.map((col, i) => ({
      ...col,
      id: col.column_name, // use the column name as unique id
      index: i,
      include_in_forms:
  ...
  // Store for the currently selected column.
  let selectedColumn = writable(null)

  // Table-level settings.
  let userFacingName = user_facing_name
  let tableDescription = table_description

  // Reactive value for the number of columns.
  $: columnCount = get(columnList).length

  // Automatically select the first column (if any) when the page loads.
  $: if (!get(selectedColumn) && get(columnList).length > 0) {
    selectedColumn.set(get(columnList)[0])
  }
  ...
  // Update the selected column when a user clicks an item in the left column.
  function selectColumn(col) {
    selectedColumn.set(col)
  }

  // Allowed tally mapping options and extra settings for each Postgres data type.
  ...
  // Determine the allowed mapping for the currently selected column.
  $: allowedMapping = $selectedColumn
  ...
  // Handler to update an extra setting for the selected column.
  function updateExtraField(fieldName, value) {
    selectedColumn.update((col) => {
      col.tally_specific_options = {
        ...(col.tally_specific_options || {}),
        [fieldName]: value,
      }
      return col
  ...
  <div class="flex space-x-4">
    ...
    <div class="w-2/3 pl-2">
      {#if $selectedColumn}
        ...
        <!-- Tally Field Type, now restricted based on Postgres type -->
        <div class="mb-4">
          <label
            class="block font-medium"
        ...
        <!-- Extra Settings: display only if there are extra fields defined for this tally block -->
